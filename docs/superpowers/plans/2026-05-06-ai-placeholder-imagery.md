...
**Goal:** Populate the 8 currently-empty hero image slots (1 home + 7 programs) with AI-generated placeholder imagery, using the Sanity MCP `generate_image` tool, without schema changes or new dependencies.
...
Write `scripts/image-prompts.mjs` with the exact contents below.
